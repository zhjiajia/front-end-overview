## 写在这里
一篇文章写下来总是有原因的，但就像作者作答自己文章的阅读理解总是得不到满分一样，所以从几方面聊一聊。
### 适合谁阅读，有什么内容
* 对于一个从未了解过计算机的人，这篇文章甚至都是合适的，因为它从最基础的实用性开始，一点点解释了为什么要前端。

如果你不习惯观看这样的md文档，你也可以下载vscode，这样打开的.md文件是编辑模式，或许更方便阅读。
### 为啥要写这篇文章
...
### 期望您的帮助
在十一期间我写下了最开始的版本，由于精力有限，只完成了少数部分。
这篇文章以开源的形式发布在Github上，期待您向它进行:内容扩展、错误指正。

## 计算机世界
计算机的世界是如此严谨，就像数学一样，1+1=2，没错这就是基础。
...
### 小结
总而言之，获取信息并加以利用，这就是几乎全部的你需要在计算机世界要做的内容。例如，你看到本文，并产生任何的想法。就像本节开头所说的1+1=2！
## 前端
...
* 协议 https://
    用户没有输入协议，默认使用http://作为协议，这是超文本传输协议，https是加密的http协议，更加安全
...
##### 正题
我推荐你观看 https://www.bilibili.com/video/BV1x54y1B7RE，里面有所有的内容。如果这个视频因为某些原因不在了，下面是我的笔记：
```
...
#### js
##### 三方库
使用一些库无疑是提升效率的好方式，例如[lodash](https://www.lodashjs.com/),只需要引入，然后使用。
##### 框架
...
### 聊聊浏览器API
#### 运行时
js代码的执行环境(js runtime environment)提供了一些基本设施和功能，如浏览器和Node.js，更具体来说是指其内的js引擎，如Chrome的V8引擎。
...
#### 如何学习
去官网就好了，里面应该提到了完整的周边生态，然而对于新手还需要跟几个demo，同时注意版本
#### 它是如何做到的？
在前些年，阅读vue和react的源码着实火了一阵，一些文章也很优秀，例如
### 构建生产环境
#### 直接使用框架
...
#### 备份
几年前的项目只剩打包好的版本，原型找不到等等，我们可不想遇见这样的情况。
#### 团队合作
##### 通用(以UI举例)
